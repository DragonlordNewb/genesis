...
# = Basic neural networking classes = #
...
class Neuron:
	def __init__(self):
		self.bias = generateNormal(0, 2)

		self.inputAxons = [] # axons leading into this neuron
		self.outputAxons = [] # axons leading from this neuron
	...
	def receiveInput(self, value: Scalar) -> None:
		self.inputs.append(value)

	def receiveError(self, error: Scalar) -> None:
		self.error += error
		self.bias /= error

	def feedforward(self) -> None:
		for axon in self.outputAxons:
			axon.feedforward(self.compute())
...
class Axon:
	def __init__(self, inputNeuron: Neuron, outputNeuron: Neuron) -> None:
		self.input = inputNeuron # takes input from input neuron
		self.input.outputAxons.append(self)
		self.output = output # puts output into output neuron
		self.output.inputAxons.append(self)

		self.weight = generateNormal(0, 2)

	def feedforward(self, value: Scalar) -> None:
		self.output.receiveInput(value * self.weight)

	def backpropagate(self, error: Scalar) -> None:
		self.input.receiveError(error * self.weight)
		self.weight /= error

# = Feedforward neural networking = #

class FeedforwardLayer:
	def __init__(self, parent: object, *neurons: set[Neuron]) -> None:
	...
	def output(self) -> list[Scalar]:
		return [neuron.compute() for neuron in self]
	...
	@classmethod
	def fromCount(cls, count: int) -> object:
		return cls(*[Neuron() for _ in range(count)])

class FeedforwardInputLayer(FeedforwardLayer):
	def receiveInputs(self, *inputs: set[Scalar]) -> None:
		for neuron in self:
			for value in inputs:
				neuron.receiveInput(value)

class FeedforwardOutputLayer(FeedforwardLayer):
	def receiveErrors(self, *errors: set[Scalar]) -> None:
		for neuron in self:
			for error in errors:
				neuron.receiveError(error)

class FeedforwardHiddenLayer(FeedforwardLayer):
	pass
...
class FeedforwardNeuralNetwork:
	def __init__(self, *layers: set[Union[FeedforwardInputLayer, FeedforwardHiddenLayer, FeedforwardOutputLayer]]) -> None:
		...
		for layer in self[INPUT + 1:len(self) - 1]:
			assert type(layer) == FeedforwardHiddenLayer, "Middle layers must be hidden layers."
	...
	def feedforward(self, inputs: list[Scalar]) -> list[Scalar]:
